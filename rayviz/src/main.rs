...
fn main() {
    ...
    let mut render_instant = SystemTime::now();
    let mut render_delta = Duration::new(0, 0);
    let mut sim_delta = Duration::new(0, 0);
    ...
    while !rl.window_should_close() {
        ...
        d.clear_background(Color::BLACK);
        {
            ...
            render_delta = render_instant.elapsed().unwrap_or(Duration::new(0, 0));
            let sim_instant = SystemTime::now();
            if calc_next {
                sg.step(&mut rnd);
            }
            sim_delta = sim_instant.elapsed().unwrap_or(Duration::new(0, 0));
            render_instant = SystemTime::now();
...
Simtime:   {:>3}ms",
                &d.get_fps(),
                &render_delta.as_millis(),
                &sim_delta.as_millis(),
            );
            draw_text(&mut d, &font, 5, 5, &infostring);
